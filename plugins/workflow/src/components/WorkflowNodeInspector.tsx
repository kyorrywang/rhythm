...
export function WorkflowNodeInspector({ payload }: OverlayProps<WorkflowNodeInspectorPayload>) {
  return (
    <div className="h-full overflow-y-auto bg-[#fbfaf7] p-6">
      ...
      <section className="mt-6 rounded-[var(--theme-radius-card)] border border-slate-200 bg-white p-4 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-900">Config</h3>
        <pre className="mt-3 overflow-auto rounded-[var(--theme-radius-control)] bg-slate-950 p-3 text-xs leading-5 text-slate-100">
          {JSON.stringify(payload.node.config, null, 2)}
        </pre>
      </section>

      {payload.nodeRun && (
        <section className="mt-4 rounded-[var(--theme-radius-card)] border border-slate-200 bg-white p-4 shadow-sm">
