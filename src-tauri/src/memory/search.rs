...
pub fn find_relevant_memories(
    ...
    let mut scored: Vec<(f64, MemoryHeader)> = headers.into_iter().filter_map(|h| {
    ...
    scored.sort_by(|a, b| {
        b.0.partial_cmp(&a.0)
            ...
            .then(b.1.modified_at.partial_cmp(&a.1.modified_at).unwrap_or(std::cmp::Ordering::Equal))
    });

    scored.into_iter().take(max_results).map(|(_, h)| h).collect()
}
