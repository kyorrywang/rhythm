...
use crate::domains::cron::types::{compute_next_run, CronJob};
use crate::platform::paths::get_cron_registry_path;
...
impl CronRegistry {
        ...
        self.jobs
            ...
            .filter(|job| {
                job.enabled
                    ...
                    && !matches!(
                        job.last_status,
                        Some(crate::domains::cron::types::CronRunStatus::Running)
                    )
            })
            .cloned()
            .collect()
    ...
    pub fn claim_due_jobs(&mut self, now: chrono::DateTime<chrono::Utc>) -> Vec<CronJob> {
        ...
        for job in &due_jobs {
            if let Some(stored) = self.jobs.get_mut(&job.id) {
                stored.last_status = Some(crate::domains::cron::types::CronRunStatus::Running);
    ...
    pub fn update_after_run(
        ...
        id: &str,
        last_run: f64,
        last_status: crate::domains::cron::types::CronRunStatus,
    ) {
        if let Some(job) = self.jobs.get_mut(id) {
