...
impl ToolRegistry {
    ...
    pub fn create_with_mcp(
        mcp_manager: Option<Arc<Mutex<crate::platform::mcp::McpClientManager>>>,
    ) -> Self {
        Self::create_with_plugins_and_mcp(&[], mcp_manager)
    }

    pub fn create_with_plugins_and_mcp(
        plugins: &[crate::domains::plugins::LoadedPlugin],
        mcp_manager: Option<Arc<Mutex<crate::platform::mcp::McpClientManager>>>,
    ) -> Self {
        ...
        for plugin in plugins {
            ...
            for declaration in &plugin.manifest.contributes.agent_tools {
                if let Some(tool) = crate::domains::plugins::PluginToolAdapter::from_manifest(
                    plugin.name(),
                    plugin.path.clone(),
                    declaration,
                    plugins,
        ...
        if let Some(manager) = mcp_manager {
            ...
            for tool_info in tool_infos {
                registry.register(Box::new(crate::platform::mcp::McpToolAdapter::new(
                    manager.clone(),
                    &tool_info,
                )));
            }
