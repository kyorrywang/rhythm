...
use crate::domains::chat::prompts::build_runtime_prompt_with_addition;
use crate::domains::session::session_tree;
use crate::domains::swarm::agent_registry;
use crate::platform::config;
use crate::platform::event_bus;
use crate::platform::llm;
use crate::platform::mcp::McpClientManager;
use crate::shared::schema::EventPayload;
use async_trait::async_trait;
use serde::Deserialize;
use serde_json::Value;

#[path = "subagent_support.rs"]
mod support;

use support::{current_time_millis, resolve_delegation_target, wait_for_subagent_interrupt};
...
impl BaseTool for SubagentTool {
    ...
    fn parameters(&self) -> Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "agent_id": {
                    "type": "string",
                    "description": "The configured agent id to delegate to (for example 'dynamic' or 'explorer')"
                }
            },
    ...
    async fn execute(&self, args: Value, ctx: &ToolExecutionContext) -> ToolResult {
        ...
        use crate::domains::chat::engine::query_engine::QueryEngine;
        use crate::domains::chat::hooks::executor::HookExecutor;
        use crate::domains::chat::hooks::loader::load_hook_registry_for_cwd;
        ...
        let inherited_provider_id = ctx
            ...
            .and_then(|value| value.as_str())
            .map(str::to_string);
        let inherited_model = agent_def.model.model_id.clone().or_else(|| {
            ctx.metadata
                .get("model")
                .and_then(|value| value.as_str())
                .map(str::to_string)
        ...
        let injected_subagent_prompt = Some(config::render_prompt_fragments(
            &settings,
            &agent_def.prompt_refs,
        ))
        .filter(|prompt| !prompt.trim().is_empty());
        let merged_additional_prompt = match (
            injected_subagent_prompt.as_deref(),
            ...
            (_, Some(system_prompt)) if !system_prompt.trim().is_empty() => {
        ...
        let tool_registry =
            std::sync::Arc::new(crate::domains::tools::ToolRegistry::create_for_agent(
                mcp_manager.clone(),
                allowed_tools,
                denied_tools,
            ));
        let permission_checker = std::sync::Arc::new(
            crate::domains::permissions::checker::PermissionChecker::new(&runtime_spec.permission),
        );
        ...
        let context = crate::domains::chat::engine::context::QueryContext {
        ...
        let result: Result<String, crate::shared::error::RhythmError> = tokio::select! {
            ...
            _ = wait_for_subagent_interrupt(&ctx.session_id, &sub_session_id) => {
                let _ = crate::domains::chat::interrupts::request_interrupt(&sub_session_id).await;
...
mod tests {
    use super::*;
    use crate::platform::config::{
        AgentConfigKind, AgentDefinitionConfig, AgentExecutionConfig, AgentModelConfig,
        AgentPermissions,
    };
    ...
    fn resolve_delegation_target_returns_subagent_agent() {
        ...
        let agent =
            resolve_delegation_target(&settings, "test_subagent").expect("test_subagent target");
