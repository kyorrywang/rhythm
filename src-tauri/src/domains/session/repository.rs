use crate::platform::database::Database;
...
pub enum MessageSegmentSnapshot {
    ...
    Ask {
        ...
        status: String,
        answer: Option<AskResponseSnapshot>,
        started_at: Option<i64>,
        ended_at: Option<i64>,
    },
    #[serde(rename = "tasks")]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct AskResponseSnapshot {
    pub answers: Vec<AskAnswerSnapshot>,
}
...
pub fn sanitize_session(mut session: SessionSnapshot) -> SessionSnapshot {
    session.messages = session.messages.into_iter().map(sanitize_message).collect();
...
pub async fn list_sessions(cwd: &Path) -> Result<Vec<SessionSnapshot>, String> {
    ...
    for row in rows {
        ...
        match serde_json::from_str::<SessionSnapshot>(&snapshot) {
            ...
            Err(error) => {
                eprintln!(
                    "Skipping invalid session snapshot while listing sessions: {}",
                    error
                );
            }
        }
