...
use crate::domains::chat::engine::{QueryContext, QueryEngine};
...
use crate::domains::chat::prompts::builder::build_runtime_prompt;
use crate::domains::permissions::PermissionChecker;
use crate::domains::session::{history, session_tree, sessions};
use crate::domains::swarm::agent_registry;
use crate::domains::tools::ToolRegistry;
use crate::domains::workspace::application::resolve_workspace_path;
use crate::platform::config;
use crate::platform::event_bus;
use crate::platform::llm;
use crate::platform::llm::{ChatAttachment, LlmClient};
...
pub async fn chat_stream(
    ...
    let runtime_spec = config::resolve_runtime_spec(
        ...
        config::RuntimeIntent {
            ...
            permission_mode: permission_mode
                .as_deref()
                .map(crate::domains::permissions::modes::PermissionMode::from_str),
    ...
    tokio::spawn(async move {
        ...
        let loaded_plugins = crate::domains::plugins::load_plugins(&settings, &cwd_path);
        ...
        {
            Some(command_name) => {
                let descriptor = match crate::domains::slash::registry::resolve_slash_command(
                    &cwd_path,
                    command_name,
                ) {
                ...
                match crate::domains::slash::router::execute_slash_command(
                    &descriptor,
                    &prompt,
                    &crate::domains::slash::types::SlashRuntimeExecutionContext {
                ...
                {
                    Ok(
                        crate::domains::slash::router::SlashExecutionOutcome::ContinueWithPrompt(
                            prompt,
                        ),
                    ) => prompt,
                    Ok(crate::domains::slash::router::SlashExecutionOutcome::Handled) => {
                        emit_runtime_status(
                            &agent_id,
                            &session_id,
                            "completed",
