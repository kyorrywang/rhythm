...
use crate::platform::config::{ResolvedAgentSpec, RhythmSettings};
...
pub fn build_runtime_prompt_with_addition(
    ...
    if settings.core.memory.enabled {
        if let Some(mem_section) = crate::domains::memory::memdir::load_memory_prompt(
            cwd,
            ...
            if let Some(query) = latest_user_prompt {
                let relevant = crate::domains::memory::search::find_relevant_memories(
                    query,
                    cwd,
...
fn build_skills_section(settings: &RhythmSettings, cwd: &Path) -> String {
    let registry = crate::platform::skills::loader::load_skill_registry_for_cwd(&settings, &cwd);
    build_skills_section_from_registry(&registry)
}

fn build_skills_section_from_registry(registry: &crate::platform::skills::SkillRegistry) -> String {
