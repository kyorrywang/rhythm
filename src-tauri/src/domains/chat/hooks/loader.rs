...
use crate::platform::config::RhythmSettings;
use crate::platform::config::{HookConfig, HooksConfig};
...
pub fn load_hook_registry_for_cwd(settings: &RhythmSettings, cwd: &Path) -> HookRegistry {
    let mut registry = load_hook_registry(&settings.core.hooks);

    for plugin in crate::domains::plugins::load_plugins(settings, cwd) {
