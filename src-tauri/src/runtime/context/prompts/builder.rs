...
use crate::infra::config::{ResolvedAgentSpec, RhythmSettings};
...
pub fn build_runtime_prompt(
    ...
    build_runtime_prompt_with_addition(settings, cwd, latest_user_prompt, None, runtime_spec)
}
...
pub fn build_runtime_prompt_with_addition(
    ...
    if settings.core.memory.enabled {
        if let Some(mem_section) = crate::runtime::context::memory::memdir::load_memory_prompt(
            cwd,
            settings.core.memory.max_entrypoint_lines,
        ) {
            sections.push(mem_section);

            // Relevant memories based on the latest user prompt
            if let Some(query) = latest_user_prompt {
                let relevant = crate::runtime::context::memory::search::find_relevant_memories(
                    query,
                    cwd,
    ...
    if !runtime_spec.delegate_agents.is_empty() {
        let mut lines = vec![
            ...
            "The following delegate agents are available in this session. Use them when helpful."
                .to_string(),
            String::new(),
        ];
...
fn build_skills_section(settings: &RhythmSettings, cwd: &Path) -> String {
    let registry =
        crate::runtime::capabilities::skills::loader::load_skill_registry_for_cwd(&settings, &cwd);
    build_skills_section_from_registry(&registry)
}

fn build_skills_section_from_registry(
    registry: &crate::runtime::capabilities::skills::SkillRegistry,
) -> String {
