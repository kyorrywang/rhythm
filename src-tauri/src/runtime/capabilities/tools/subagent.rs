...
use crate::runtime::context::prompts::build_runtime_prompt_with_addition;
use crate::runtime::conversation::session::session_tree;
...
impl BaseTool for SubagentTool {
    ...
    async fn execute(&self, args: Value, ctx: &ToolExecutionContext) -> ToolResult {
        ...
        use crate::runtime::conversation::engine::query_engine::QueryEngine;
        ...
        let tool_registry = std::sync::Arc::new(
            crate::runtime::capabilities::tools::ToolRegistry::create_for_agent(
                mcp_manager.clone(),
                allowed_tools,
                denied_tools,
            ),
        ...
        let permission_checker = std::sync::Arc::new(
            crate::runtime::policy::permissions::checker::PermissionChecker::new(
                &runtime_spec.permission,
            ),
        );
        ...
        let context = crate::runtime::conversation::engine::context::QueryContext {
        ...
        let result: Result<String, crate::shared::error::RhythmError> = tokio::select! {
            ...
            _ = wait_for_subagent_interrupt(&ctx.session_id, &sub_session_id) => {
                let _ = crate::runtime::conversation::interrupts::request_interrupt(&sub_session_id).await;
