...
use crate::runtime::capabilities::slash::host_api::PluginRuntimeHost;
use crate::runtime::extensions::tool_adapter::{
    run_plugin_runtime, PluginRuntimeCall, PluginRuntimeCallContext,
};

use super::super::router::SlashExecutionOutcome;
use super::super::types::{
    PluginSlashRuntimeRequest, SlashCommandDescriptor, SlashRuntimeExecutionContext,
    SlashRuntimeInput,
};
...
pub async fn execute_plugin_command(
    ...
    let plugins = crate::runtime::extensions::loader::load_plugins(&settings, cwd_path);
    let plugin = plugins
        .iter()
        .find(|plugin| plugin.name() == plugin_name && plugin.is_runtime_active())
        .ok_or_else(|| format!("Slash plugin '{}' is not active", plugin_name))?;
    let slash = plugin.slash_contribution.as_ref().ok_or_else(|| {
        format!(
            "Slash plugin '{}' does not declare contributes.slash",
            plugin_name
        )
    })?;
    ...
    let request = PluginSlashRuntimeRequest {
        ...
        slash: crate::runtime::capabilities::slash::types::PluginSlashContributionRuntimeConfig {
