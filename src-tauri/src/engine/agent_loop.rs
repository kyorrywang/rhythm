...
use super::compactor::{AutoCompactState, auto_compact_if_needed};
...
pub async fn run_query(
    ...
    if let Some(hook_exec) = &context.hook_executor {
        ...
        if hook_result.blocked() {
            ...
            return Err(RhythmError::HookError(format!("SessionStart hook blocked: {}", hook_result.reason())));
        }
    }
...
async fn run_query_inner(
    ...
    let tool_defs: Vec<LlmToolDefinition> = context.tool_registry.to_api_schema()
        .into_iter()
    ...
    for turn in 0..context.max_turns {
        ...
        if context.auto_compact_enabled {
            ...
            if compact_result.was_compacted {
                *messages = compact_result.messages;
                let compact_type_str = compact_result.compact_type
                    .as_ref()
        ...
        while let Some(res) = stream.next().await {
            match res {
                Ok(LlmResponse::ThinkingDelta(delta)) => {
                    ...
                    event_bus::emit(&context.agent_id, &context.session_id,
                        EventPayload::ThinkingDelta { content: delta.clone() });
                }
                Ok(LlmResponse::TextDelta(delta)) => {
                    if !thinking_ended {
                        let elapsed = thinking_started_at
                            ...
                            .unwrap_or(0);
                        event_bus::emit(&context.agent_id, &context.session_id,
                            EventPayload::ThinkingEnd { time_cost_ms: elapsed });
                        thinking_ended = true;
                    }
                    event_bus::emit(&context.agent_id, &context.session_id,
                        EventPayload::TextDelta { content: delta.clone() });
        ...
        if !pending_tool_calls.is_empty() {
            ...
            if interrupts::is_interrupted(&context.session_id).await {
                interrupts::clear_interrupt(&context.session_id).await;
                event_bus::emit(&context.agent_id, &context.session_id, EventPayload::Interrupted);
                return Ok(assistant_text.clone());
            }
...
async fn execute_tools(
    ...
    let tool_call_blocks: Vec<ChatMessageBlock> = tool_calls.iter().map(|tc| {
    ...
    for tool_call in &tool_calls {
        ...
        event_bus::emit(&context.agent_id, &context.session_id, EventPayload::ToolStart {
        ...
        let is_error = matches!(&result_block, ChatMessageBlock::ToolResult { is_error, .. } if *is_error);

        event_bus::emit(&context.agent_id, &context.session_id, EventPayload::ToolEnd {
            tool_id: tool_call.id.clone(),
    ...
    ToolExecutionBatch { tool_call_blocks, tool_results }
}
    ...
    if !decision.allowed {
        if decision.requires_confirmation {
            if coordinator_mode::is_swarm_worker() {
                ...
                let worker_id = coordinator_mode::get_agent_id().unwrap_or_else(|| context.agent_id.clone());
                ...
                match crate::swarm::permission_sync::wait_for_permission_response(&team_name, &request.id, 300_000).await {
                    ...
                    Ok(response) => {
                        return ChatMessageBlock::ToolResult {
                            ...
                            content: format!(
                                ...
                                tool_name,
                                response.feedback.unwrap_or_else(|| "leader rejected".to_string())
                            ),
                            is_error: true,
                        };
                    }
                ...
                event_bus::emit(&context.agent_id, &context.session_id, EventPayload::PermissionRequest {
                ...
                match rx.await {
                    ...
                    _ => {
                        return ChatMessageBlock::ToolResult {
                            tool_call_id: tool_id.clone(),
                            content: format!("Permission denied for '{}': user rejected", tool_name),
                            is_error: true,
                        };
                    }
                }
