use super::runtime::normalize_config_bundle;
use super::validation::validate_config_bundle;
use super::*;
...
pub(super) fn load_config_bundle() -> ConfigBundle {
    ...
    let (mut bundle, migrated) = match migration::upgrade_config_bundle(raw_value) {
        Ok(result) => result,
        Err(error) => {
    ...
    let agent_definitions = match crate::runtime::agents::load_all_agent_definitions(None) {
    ...
    crate::runtime::agents::merge_agent_definitions_into_settings(&mut bundle, &agent_definitions);
...
pub(super) fn create_default_config(path: &std::path::Path) -> ConfigBundle {
    ...
    if let Ok(agent_definitions) = crate::runtime::agents::load_all_agent_definitions(None) {
        crate::runtime::agents::merge_agent_definitions_into_settings(
            &mut bundle,
            &agent_definitions,
        );
    }
...
pub(super) fn save_config_bundle(bundle: &ConfigBundle) -> Result<(), String> {
    ...
    let agent_definitions = crate::runtime::agents::load_all_agent_definitions(None)
        .unwrap_or_else(|_| crate::runtime::agents::default_agent_definitions());
    crate::runtime::agents::strip_agent_data_from_settings(&mut persisted, &agent_definitions);
