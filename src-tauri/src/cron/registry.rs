...
use crate::cron::types::{compute_next_run, CronJob};
use crate::infrastructure::paths::get_cron_registry_path;
...
impl CronRegistry {
    ...
    pub fn claim_due_jobs(&mut self, now: chrono::DateTime<chrono::Utc>) -> Vec<CronJob> {
        ...
        for job in &due_jobs {
            if let Some(stored) = self.jobs.get_mut(&job.id) {
                stored.last_status = Some(crate::cron::types::CronRunStatus::Running);
    ...
    pub fn update_after_run(
        ...
        id: &str,
        last_run: f64,
        last_status: crate::cron::types::CronRunStatus,
    ) {
        if let Some(job) = self.jobs.get_mut(id) {
