...
use crate::platform::config::{HistoryToolResultsMode, LlmConfig};
                    ...
                    ChatMessage {
                        ...
                        blocks: message
                            ...
                            .filter(|block| match block {
                                ChatMessageBlock::ToolCall { id, .. } => allowed_ids.contains(id),
...
mod tests {
    use super::*;
    use crate::platform::config::{HistoryToolResultsMode, ModelCapabilities};
    use serde_json::json;
    ...
    fn sample_config(mode: HistoryToolResultsMode, tools: Option<Vec<&str>>) -> LlmConfig {
        LlmConfig {
            ...
            capabilities: ModelCapabilities {
                ...
                history_tool_result_tools: tools
    ...
    fn drops_all_tool_results_when_configured() {
        ...
        assert!(normalized[0].blocks.iter().all(|block| !matches!(
            block,
            ChatMessageBlock::ToolCall { .. } | ChatMessageBlock::ToolResult { .. }
        )));
    }
