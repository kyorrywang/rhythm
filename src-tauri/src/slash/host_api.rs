...
use crate::plugins::types::LoadedPlugin;
use crate::tools::{BaseTool, ToolExecutionContext};
...
pub async fn handle_runtime_rpc(value: &Value, host: Option<&PluginRuntimeHost<'_>>) -> Value {
    ...
    match method {
        "command.execute" => {
            ...
            let registry =
                crate::plugins::command_registry::PluginCommandRegistry::from_plugins(host.plugins);
        ...
        "workspace.readText" => {
            ...
            match crate::tools::context::resolve_and_validate_path(&host.cwd.to_path_buf(), path)
                .and_then(|resolved| {
                    if !resolved.exists() {
                        return Ok(None);
                    }
                ...
                Ok(resolved) => {
                    if let Some(parent) = resolved.parent() {
                        if let Err(error) = std::fs::create_dir_all(parent) {
                            return rpc_error(id, format!("Cannot create directory for '{}': {}", path, error));
                        }
                    }
                    match std::fs::write(&resolved, content.as_bytes()) {
                        Ok(_) => serde_json::json!({ "id": id, "ok": true, "data": { "path": path } }),
                        Err(error) => rpc_error(id, format!("Cannot write file '{}': {}", path, error)),
                    }
                }
                Err(error) => rpc_error(id, error),
            }
                    ...
                    match std::fs::read_dir(&resolved) {
                        Ok(items) => {
                            for entry in items.flatten() {
                                ...
                                let kind = if entry_path.is_dir() { "directory" } else { "file" };
                        ...
                        Err(error) => rpc_error(id, format!("Cannot read directory '{}': {}", path, error)),
                    }
                }
                Err(error) => rpc_error(id, error),
            }
        ...
        "pluginStorage.readText" => {
            ...
            match resolve_plugin_storage_path(&storage_root, path).and_then(|resolved| {
                ...
                std::fs::read_to_string(&resolved)
                    .map(Some)
                    .map_err(|error| format!("Cannot read plugin storage file '{}': {}", path, error))
            }) {
                Ok(content) => serde_json::json!({ "id": id, "ok": true, "data": content }),
                Err(error) => rpc_error(id, error),
            }
        ...
        "pluginStorage.listFiles" => {
            ...
            match resolve_plugin_storage_path(&storage_root, path) {
                Ok(resolved) => {
                    ...
                    if !resolved.is_dir() {
                        return rpc_error(id, format!("Plugin storage path '{}' is not a directory", path));
                    }
                    match std::fs::read_dir(&resolved) {
                        Ok(entries) => {
        ...
        "askUser" => {
            ...
            match crate::tools::ask::AskTool::execute_structured(params, &ctx).await {
        ...
        "task.update" => {
            ...
            let tasks: Vec<crate::shared::schema::Task> = match serde_json::from_value(tasks_value) {
                Ok(tasks) => tasks,
                Err(error) => return rpc_error(id, format!("Invalid task update payload: {}", error)),
            };
            crate::infrastructure::event_bus::emit(
                agent_id,
                session_id,
                crate::shared::schema::EventPayload::TaskUpdate { tasks: tasks.clone() },
            );
        ...
        "spawnSubagent" => {
            ...
            let params = value
                ...
                .cloned()
                .unwrap_or_else(|| serde_json::json!({}));
            let tool = crate::tools::subagent::SubagentTool;
...
fn resolve_plugin_storage_path(storage_root: &PathBuf, relative: &str) -> Result<PathBuf, String> {
    if relative.trim().is_empty() {
        return Err("Plugin storage path cannot be empty".to_string());
    }
    crate::tools::context::resolve_and_validate_path(storage_root, relative)
}
