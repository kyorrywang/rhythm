...
impl OrchestratorRuntime {
    ...
    pub async fn handle_chat_stream(
        ...
        let llm_config = config.get("llm").and_then(|l| l.as_object());
        
        let api_key = llm_config.and_then(|l| l.get("key")).and_then(|v| v.as_str()).unwrap_or("dummy").to_string();
        let model = llm_config.and_then(|l| l.get("model")).and_then(|v| v.as_str()).unwrap_or("gpt-4o").to_string();
        let base_url = llm_config.and_then(|l| l.get("url")).and_then(|v| v.as_str()).map(|s| s.to_string());
